...
async def test_db_save_with_real_flow(db_session: AsyncSession):
    ...
    with patch("tronpy.tron.Tron.get_account", return_value=mock_account_data), \
        ...
        result = await db_session.execute(select(RequestLog).where(
            RequestLog.address == "TNPeeaaFB7K9cmo4uQpcU32zGK8G1NYqeL"))
...
async def test_multiple_log_entries(db_session: AsyncSession):
    ...
    logs = [
        RequestLog(address=f"test_address_{i}", bandwidth=i * 10, energy=i * 5, balance=i * 100)
        for i in range(1, 4)
    ]
    ...
    # запрос данных из БД
    result = await db_session.execute(select(RequestLog).order_by(RequestLog.address))
...
async def test_required_fields(db_session: AsyncSession):
    ...
    test_log = RequestLog(address="required_only")
